...
versionElement.innerText = `tsukirxpp! v${manifest.version}`

// Set after the extension initializes, used for additional error information.
let currentUrl: string
let cleanBeatmap: rxpp.beatmap
...
const calculate = async () => {
  try {
    ...
    let bpmMultiplier = 1
    if (modifiers & rxpp.modbits.dt) bpmMultiplier = 1.5
    if (modifiers & rxpp.modbits.ht) bpmMultiplier = 0.75
    ...
    switch (cleanBeatmap.mode) {
      case MODE_STANDARD:
        ...
        // If API fails, use rxpp to calculate pp.
        if ((!pp || pp == undefined || isNaN(Number(pp))) && (!stars || stars == undefined || isNaN(Number(stars)))) {
          stdResult = std.calculatePerformance(
            cleanBeatmap,
            modifiers,
            combo,
            misses,
          ...
          pp = stdResult.pp.total
          stars = stdResult.stars.total
          arElement.innerText =
            cleanBeatmap.ar === undefined
              ? '?'
              : (
    ...
    difficultyStarsElement.innerText = Number(stars).toFixed(2)
    bpmElement.innerText = (Math.round(bpm * 10) / 10).toString()

    setResultText(Math.round(pp))
  } catch (error) {
    displayError(error)
  }
}
...
const getPageInfo = (url: string, tabId: number): Promise<PageInfo> =>
  new Promise((resolve, reject) => {
    ...
    if (match[1] == "tsuki.host" && match[2] == "b") {
      const mode = "osu"
      const beatmapId = match[3]
	  
      info.mode = mode
      info.beatmapId = beatmapId
	  
	  chrome.tabs.create({ url: `https://osu.ppy.sh/b/${beatmapId}`, active: false }, async (tab: any) => {
      await sleep(5)
      chrome.tabs.sendMessage(
        ...
        (response) => {
          ...
          info.beatmapSetId = response.beatmap.beatmapset_id
...
const processBeatmap = (rawBeatmap: string) => {
  const { map } = new rxpp.parser().feed(rawBeatmap)
