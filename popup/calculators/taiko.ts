...
export const calculate = (
  ...
  let clockRate = 1
  if (mods & rxpp.modbits.dt) clockRate = 1.5
  if (mods & rxpp.modbits.ht) clockRate = 0.75
  if (mods & rxpp.modbits.hr) {
    const ratio = 1.4
    map.od = Math.min(map.od * ratio, 10.0)
  }
  if (mods & rxpp.modbits.ez) {
    const ratio = 0.5
    map.od *= ratio
  }
...
export const calculatePerformance = (
  ...
  let multiplier = 1.1
  if (mods & rxpp.modbits.nf) multiplier *= 0.9
  if (mods & rxpp.modbits.hd) multiplier *= 1.1
...
export const calculateStrainPerformance = (
  ...
  if (mods & rxpp.modbits.hd) strainValue *= 1.025
  if (mods & rxpp.modbits.fl) strainValue *= 1.05 * lengthBonus
